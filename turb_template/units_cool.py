...
frac = 10e-4
unit_length = CONST_pc * 1e3 * frac # 1 kpc
unit_time = CONST_yr * 1e6 * frac # 1 Myr
...
def cs_calc(T, mu=mu):
    import numpy as np
    return np.sqrt((CONST_kB * T) / (mu * CONST_amu)) / unit_velocity

def pressure(rho, T, mu_val=mu):
    """Ideal gas pressure in code units: p = rho * T / (KELVIN * mu)"""
    return (rho * T) / (KELVIN * mu_val)
